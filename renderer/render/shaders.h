...
class CqLayeredShader : public IqShader
{
	public:
		...
		virtual IqShaderData* CreateVariable( EqVariableType Type, EqVariableClass Class, const CqString& name, bool fArgument = false, bool fOutput = false, TqInt varyingSize = 1  )
		{
			...
			if(!m_Layers.empty())
				return(m_Layers.front().second->CreateVariable(Type, Class, name, fArgument, fOutput, varyingSize));
			return ( NULL );
		}
		virtual IqShaderData* CreateVariableArray( EqVariableType Type, EqVariableClass Class, const CqString& name, TqInt Count, bool fArgument = false, bool fOutput = false, TqInt varyingSize = 1  )
		{
			// Call CreateVariableArray on the first shader in the list, all layers must be the same type.
			if(!m_Layers.empty())
				return(m_Layers.front().second->CreateVariableArray(Type, Class, name, Count, fArgument, fOutput, varyingSize));
			return ( NULL );
		}
